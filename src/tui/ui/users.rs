...
fn render_user_list(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let rows: Vec<Row> = app.users_state.users.iter().enumerate().map(|(i, user)| {
        ...
        let last_active = user.last_activity
...
fn render_user_stats(frame: &mut Frame, app: &App, area: Rect) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(&stats.user_id, Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
    ]));

    lines.push(Line::from(""));
    lines.push(Line::from(Span::styled("-- API Usage --", Style::default().fg(Color::Cyan))));

    lines.push(Line::from(vec![
        Span::raw("Total Cost:     "),
        Span::styled(format_currency(stats.total_cost), Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
    ]));

    lines.push(Line::from(vec![
        Span::raw("Total Tokens:   "),
        Span::styled(format_tokens(stats.total_tokens), Style::default().fg(Color::Yellow)),
    ]));

    lines.push(Line::from(vec![
        Span::raw("API Calls:      "),
        Span::styled(stats.total_api_calls.to_string(), Style::default().fg(Color::White)),
    ]));

    lines.push(Line::from(""));
    lines.push(Line::from(Span::styled("-- Call Breakdown --", Style::default().fg(Color::Cyan))));

    lines.push(Line::from(vec![
        Span::raw("Chat:           "),
        Span::styled(stats.chat_calls.to_string(), Style::default().fg(Color::White)),
    ]));

    lines.push(Line::from(vec![
        Span::raw("Whisper:        "),
        Span::styled(stats.whisper_calls.to_string(), Style::default().fg(Color::White)),
    ]));

    lines.push(Line::from(vec![
        Span::raw("DALL-E:         "),
        Span::styled(stats.dalle_calls.to_string(), Style::default().fg(Color::White)),
    ]));

    lines.push(Line::from(""));
    lines.push(Line::from(Span::styled("-- Activity --", Style::default().fg(Color::Cyan))));

    lines.push(Line::from(vec![
        Span::raw("Messages:       "),
        Span::styled(stats.message_count.to_string(), Style::default().fg(Color::White)),
    ]));

    lines.push(Line::from(vec![
        Span::raw("DM Sessions:    "),
        Span::styled(stats.dm_session_count.to_string(), Style::default().fg(Color::White)),
    ]));
    ...
    if let Some(first) = &stats.first_seen {
        lines.push(Line::from(vec![
            Span::raw("First Seen:     "),
            Span::styled(first.format("%Y-%m-%d").to_string(), Style::default().fg(Color::DarkGray)),
        ]));
    }
    ...
    if let Some(last) = &stats.last_activity {
        lines.push(Line::from(vec![
            ...
            Span::styled(last.format("%Y-%m-%d").to_string(), Style::default().fg(Color::DarkGray)),
        ]));
    }
...
fn render_dm_sessions(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let rows: Vec<Row> = sessions.iter().map(|session| {
        ...
        Row::new(vec![
            ...
            Cell::from(format_currency(session.api_cost)).style(Style::default().fg(Color::Green)),
